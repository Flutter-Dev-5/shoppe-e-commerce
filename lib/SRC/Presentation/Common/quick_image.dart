...
import 'package:tf_ecommerce/SRC/Application/Utils/Extensions/image_type_ext.dart';
...
class QuickImage<T> extends StatelessWidget {
  ...
  //  print(url.toString().imageType);
    //print(url.toString().toLowerCase().endsWith("svg"));
    if (url is File) {
      return Image.file(
        url as File,
      ...
      return Icon(url as IconData,size: height,);
    }
    else{
      return url.toString().imageType==ImageType.svg
          ? _buildSvgImage(context) : url.toString().imageType==ImageType.png? 
...
Widget _buildAssetImage(context){
print("in assetsssssss");
return  Image.asset(
    url.toString(),
    fit: fit ?? BoxFit.fill,
    color: color,
